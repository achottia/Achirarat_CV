...
const jpHomePageData = {
  name: "アチララット・チョッティアンチャイワット",
  jobTitle: "組込みソフトウェアエンジニア",
  home_title: "私について",
  home_content: `
  <div>
    <p> 情報通信工学の修士号と、工業電気工学（次席卒業）の学士号を持つ電気・ソフトウェアエンジニアで、自動車および鉄道信号システムにおける実務経験があります。<br>
    現在は、<b>情報理論</b>や<b>MIMO通信</b>などの通信システム、および<b>分子通信</b>や<b>バイオインフォメーション</b>を専門とする医療アプリケーションへの移行を進め、革新的なヘルスケアソリューションの開発を目指しています。技術的な専門知識には、組込みシステム、信号処理、凸最適化、およびMATLAB、C、Pythonでのプログラミングが含まれ、多様な産業において理論的な概念を実践的なエンジニアリングアプリケーションに変換する確かな能力を持っています。
    </p>
...
const jpResearchPageData = {
  ...
  content: `
    <div class='research_content'>
        <p> 私の研究は、拡散ベースのチャネルにおけるビット誤り率（BER）を厳密に分析することで、分子通信 システムの効率を向上させることに焦点を当てています。分子拡散の本質的に確率的な性質は、特にシステムの性能を分析し最適化する上で、信頼性の高い通信に大きな課題をもたらします。私の主要な目的は、これらの確率的プロセスを正確に特徴付ける包括的な拡散モデルを開発し、より効率的なMCシステムの設計を可能にすることです。具体的には、この研究は、従来の電磁気アプローチが非現実的なナノスケール環境での通信信頼性を定量化し、改善するための理論的枠組みを確立します。</p>
        ...
        <br>
        <b>概要</b> : 分子通信（MC）は、分子を情報キャリアとして使用する成長分野であり、ヘルスケア、環境モニタリング、バイオテクノロジーに応用されています。有望な方向性として、生物学的嗅覚を模倣した嗅覚に着想を得た分子通信があり、受容体の数を超える分子タイプを使用する交差反応性センサーを使用します。これにより、分子混合物を使用した効率的な伝送が可能になりますが、適切な混合物アルファベットの選択などの課題も生じます。これまでの研究は複雑な非類似度メトリクスを使用していましたが、本論文ではより単純なユークリッド距離ベースの方法を提案します。2つの定式化が探求されます。混合物の分離可能性を最大化するMax-Minアプローチと、自然な粒子間相互作用に触発された反発ベースの方法です。また、エラー率を低く抑える必要がある重要なデータのために優先メッセージも導入されます。センサーの選択については、高速二分探索と条件数最小化が評価され、精度とコスト効率の両方でランダム選択を上回ることが示されました。これらの知見は、堅牢なバイオ通信システムの将来の開発を支援するものであり、現在進行中の研究は、スケーラブルな最適化と受容体設計に焦点を当てています。
